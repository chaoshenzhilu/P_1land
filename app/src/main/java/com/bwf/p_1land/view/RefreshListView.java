...
public class RefreshListView extends ListView {
	...
	private final static float OFFSET_RADIO = 2;// 移动的比例
	...
	public RefreshListView(Context context, AttributeSet attrs) {
		...
		LogUtils.e("mHeaderViewHeight:" + mHeaderViewHeight);
		// 隐藏mHeaderView
		...
		LogUtils.e("mFooterViewHeight:" + mFooterViewHeight);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:// 移动
			...
			}else if(this.getFirstVisiblePosition()==0&&mHeaderView.getVisiableHeight()<mHeaderViewHeight){
				if(mLastY>deltaY){
		...
		case MotionEvent.ACTION_UP:// 抬起
			...
			if (getFirstVisiblePosition() == 0) {
				...
				if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
					mHeaderView.setState(Refresh_ListViewHeader.STATE_REFRESHING);
	...
	private void updateHeaderHeight2(float delta) {
		// 设置高度   移动的距离+原有的高度
		mHeaderView.setVisiableHeight(
				mHeaderView.getVisiableHeight()-(int) delta);
	}
